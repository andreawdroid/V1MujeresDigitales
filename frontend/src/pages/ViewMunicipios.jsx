...
const ViewMunicipios = () => {
  ...
  const [municipioEliminar, setMunicipioEliminar] = useState(null);

  if (municipioEliminar) {
    const confirmarEliminacion = window.confirm(
      `¿Estás seguro de eliminar el municipio "${municipioAEliminar.nombre}"?`
    );
  
    if (confirmarEliminacion) {
      // Realiza la lógica de eliminación aquí, por ejemplo, haciendo una solicitud a tu API.
      // Después de eliminar el municipio, puedes restablecer el estado a null:
      // setMunicipioAEliminar(null);
    } else {
      // Si el usuario cancela la eliminación, también restablece el estado a null:
      setMunicipioEliminar(null);
    }
  }
          ...
          <tbody>
            <tr className="border-b dark:border-neutral-500">
              ...
              <td className="px-3 py-2">
                <button
                  onClick={() => setMunicipioEliminar({ idMunicipio: 1, nombre: "Pereira" })}
                  className="text-gray-500 hover:text-black-900"
                />
                  Eliminar
              </td>
            </tr>
