...
const ViewCategorias = () => {
  // Aquí puedes agregar la lógica necesaria para obtener y mostrar las categorías

  const categorias = [
    { id: 1, nombre: "Deporte", descripcion: "Para actividades deportivas" },
    { id: 2, nombre: "Cuidado Personal", descripcion: "Productos de cuidado personal" },
  ...
  return (
    ...
    <Header/>
      <div className="text-pink-600 font-black text-4xl text-center mb-4">
        <span className="mr-4">Categorías</span>
        <Link
          to='/CrearCategorias' 
        ...
        <table className="w-2/3 md:w-1/2 text-center bg-white rounded-lg overflow-hidden">
          ...
          <tbody>
            {categorias.map((categoria) => (
              <tr className="border-b dark:border-neutral-500" key={categoria.id}>
                <td className="whitespace-nowrap px-3 py-2 font-medium">{categoria.id}</td>
                <td className="whitespace-nowrap px-3 py-2">{categoria.nombre}</td>
                <td className="whitespace-nowrap px-3 py-2">{categoria.descripcion}</td>
                <td className="px-3 py-2">
                  <Link to={`/eliminarCategoria/${categoria.id}`} className="text-gray-500 hover:text-black-900">Eliminar</Link>
                </td>
              </tr>
            ))}
          </tbody>
